...
未指定快捷命令时，继续执行现有自然语言页面生成工作流。请求开头也可以使用：

| 命令 | 目标 |
| --- | --- |
| `$ops-page-skill <需求>` / `/ops:page <需求>` | 生成运营后台页面，自动选择规格模式或 React 模式。 |
| `/ops:style <修改>` | 修改平台视觉、主题、公共 Shell 或生成规则。 |
| `/ops:review <页面>` | 按请求评审或修复页面；只要方案时不扩大为实现。 |
